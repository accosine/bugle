...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create newsroom-e85d
  fly apps create newsroom-e85d-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app newsroom-e85d
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app newsroom-e85d-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'newsroom-e85d-staging' does not match app name in config file 'newsroom-e85d'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name newsroom-e85d-db
  fly postgres attach --app newsroom-e85d newsroom-e85d-db

  fly postgres create --name newsroom-e85d-staging-db
  fly postgres attach --app newsroom-e85d-staging newsroom-e85d-staging-db
  ```
