...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create newspaper-2ffa
  fly apps create newspaper-2ffa-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app newspaper-2ffa
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app newspaper-2ffa-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'newspaper-2ffa-staging' does not match app name in config file 'newspaper-2ffa'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name newspaper-2ffa-db
  fly postgres attach --app newspaper-2ffa newspaper-2ffa-db

  fly postgres create --name newspaper-2ffa-staging-db
  fly postgres attach --app newspaper-2ffa-staging newspaper-2ffa-staging-db
  ```
